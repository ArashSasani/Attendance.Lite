...
namespace CMS.Service
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<IdentityResult> AddUserToRoleAsync(string userId, string roleId)
        {
            ...
            if (user == null || role == null)
            {
                return IdentityResult.Failed("نقش یا کاربر مورد نظر وجود ندارد.");
            }
            ...
            , string sortOrder = "")
        {
            ...
            foreach (var user in users)
            {
                dto.Add(new UserDto
                {
                    Id = user.Id,
                    Username = user.UserName,
                    UserType = user.IsPersonnel ? "پرسنل" : "کاربر سامانه",
        ...
        public async Task<IdentityResult> RegisterUserAsync(RegisterUserDto userModel)
        {
            var user = new User
            {
                ...
                IsPersonnel = userModel.IsPersonnel,
            };
        ...
        public async Task<IdentityResult> UpdateUserAsync(RegisterUserDto userModel)
        {
            ...
            if (user != null)
            {
                //personnel cannot change username
                //because it has to be linked by username to the corresponding personnel
                if (user.IsPersonnel)
                {
                    if (user.UserName != userModel.UserName)
                    {
                        return IdentityResult.Failed("نام کاربری پرسنل غیر قابل تغییر می باشد");
                    }
                }
            ...
            return IdentityResult.Failed("کاربر مورد نظر یافت نشد");
        }
